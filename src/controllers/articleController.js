...
class CreateArticle {
    CreateArticle = (async (req, res) =>{
        try{
            ...
            const blog = new Blogs({
             ...
             if(req.file){
                blog.image = `https://mybrandapis-production.up.railway.app/images/${req.file.filename}`;
             }
           const saveBlog = await blog.save();
           return res.status(200).json({message: 'Article has been added successful', data: saveBlog});
        }catch(err){
            console.log(err);
    ...
    deleteArticle = ( async(req, res) =>{
        try{
            ...
            if(blg){
                await blg.delete();
                return res.status(200).json({success: true, message: 'Article has been deleted'});
            } else{
                return res.status(404).json({success: true, message: 'Article does not found'});
            }
        }catch(err){
    ...
    updateArticle = (async(req, res) => {
        try{
            ...
            if(blg){
                ...
                if(req.file){
                    blg.image = `https://mybrandapis-production.up.railway.app/images/${req.file.filename}`;
                }
                const up = await blg.save();
                return res.status(200).json({success: true, message: "article has been updated", data: [up]});
            } 
    ...
    searchArticle = ( async(req, res) => {
        let title = req.query.article
        Blogs.findOne({title: new RegExp(title, 'i')}, function(err, doc) {
            if(doc){
